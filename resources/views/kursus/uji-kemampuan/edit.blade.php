<div id="modal-edit-uji-kemampuan" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Kursus Uji Kemampuan</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <form class="form-horizontal" id="uji-kemampuan-edit" method="post" enctype="multipart/form-data" files=true>
            @csrf
                <input type="hidden" name="id_edit">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Picture</label>
                        <div id="img-edit-uji-kemampuan" class="col-lg-9"></div>
...
$(document).ready(function(){
    ...
    $('#uji-kemampuan-edit').on('submit', function (e) {
      e.preventDefault();
        $.ajax({
            'type': 'post',
            'url' : "{{ url('admin/uji-kemampuan') }}"+"/"+$('input[name=id_edit]').val(),
            ...
            'success': function(data){
                if(data.success){
                    $('#modal-edit-uji-kemampuan').modal('hide');
